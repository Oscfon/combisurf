...
# TODO: this is copy pasted from sage-flatsurf
def are_segments_intersecting(x, y):
    r"""
    Return whether the segments ``e1`` and ``e2`` intersect.

    OUTPUT:

    - ``-1`` - at least one segment is degenerate (equal to a point)
    - ``0`` - disjoint
    - ``1`` - endpoints in common
    - ``2`` - crossing with an interior point
    - ``3`` - intersection along a segment

    EXAMPLES::

        sage: from topsurf.planar_layout import are_segments_intersecting
        sage: are_segments_intersecting((0,1,0,0),(0,0,1,3))
        0
        sage: are_segments_intersecting((0,1,0,3),(0,0,0,3))
        1
        sage: are_segments_intersecting((0,1,0,0),(0,0,-1,3))
        2
        sage: are_segments_intersecting((-1,1,0,2), (-1,1,0,2))
        3
        sage: are_segments_intersecting((-1,1,1,2),(-1,1,1,2))
        1
    """
    xs1, xt1, xs2, xt2  = x
    ys1, yt1, ys2, yt2 = y
    if xs1 == xt1 and ys1 == yt1:
        return -1
    if xs2 == xt2 and ys2 == yt2:
        return -1
    ...
    def is_valid(self, pos=None, verbose=False):
        ...
        for i1 in range(len(edges)):
            ...
            for i2 in range(i1):
                ...
                ans = are_segments_intersecting(x, y)
                if ans == -1:
                    if verbose:
                    ...
                    if verbose:
                        print(f"one-dimensional intersection between ({u1} ({pos[:,u1]}), {v1} ({pos[:,v1]})) and ({u2} ({pos[:,u2]}) ,{v2} ({pos[:,v2]}))")
                    return False
